 ...
 * its next cursor.
 */
export type PageFetcher<TItem> = (pageToken: string) => Promise<InfiniteScrollPage<TItem>>;
...
export function useInfiniteScroll<TItem>({
	...
	const [extraItems, setExtraItems] = React.useState<TItem[]>(EMPTY as TItem[]);
	const [pageToken, setPageToken] = React.useState<string | null>(initialPageToken);
