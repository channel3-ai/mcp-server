...
function useProductDetails(component: string): ProductDetailsContextValue {
	...
	if (!context) {
		throw new Error(
			`${component} must be used within <ProductDetails> or <ProductDetailsRoot>`,
		);
	}
	return context;
}
...
function Variants({ className, ...rest }: React.ComponentProps<"div">) {
	const { product, selection, onSelectVariant, isResolving, setVariantPreview } =
		useProductDetails("ProductDetailsVariants");
...
export interface ProductDetailsRecommendationsProps
	extends Omit<
		React.ComponentProps<typeof ProductRecommendations>,
...
function Recommendations({ fetchSimilar, ...rest }: ProductDetailsRecommendationsProps) {
	const {
		product,
		fetchSimilar: contextFetcher,
		recommendations,
	} = useProductDetails("ProductDetailsRecommendations");
